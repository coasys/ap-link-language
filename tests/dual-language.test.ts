 ...
 *
 * Tests the pure functions from dual-language.ts without ad4m:host runtime.
 */
...
describe("isDuplicate", () => {
    ...
    it("deduplicates based on triple only (ignores author/timestamp)", () => {
        ...
        const hash1 = linkContentHash(link1, simpleHash);
        const existing = new Set<string>([hash1]);
        // Same triple, different author/timestamp → still a duplicate
...
describe("shouldFederate", () => {
    ...
    it("returns false for ap-origin links (prevents echo loop)", () => {
        const getOrigin = (key: string): string | null => {
            if (key === "link-origin/hash789") return "ap";
            return null;
        };
