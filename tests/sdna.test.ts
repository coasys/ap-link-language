/**
 * Tests for SDNA/Subject Class pattern detection (pure logic).
 *
 * Tests the pure functions from sdna.ts without ad4m:host runtime.
 */
...
describe("detectPattern", () => {
    ...
    describe("chat-message detection", () => {
        ...
        it("does not detect chat-message for predicates not in chatPredicates", () => {
            const link = makeLink({ predicate: "flux://has_message" });
            const result = detectPattern(link, []); // empty chat predicates
            // Should fall through to unknown since flux://has_message is not in reply/reaction/content
    ...
    describe("mention detection", () => {
        ...
        it("detects 'mention' case-insensitively in predicate", () => {
            const link = makeLink({
                source: "expr://msg",
            ...
            const result = detectPattern(link, DEFAULT_CHAT_PREDICATES);
            assert.equal(result.type, "mention");
            assert.equal(result.mentionedAgent, "did:key:z6MkBob");
        });
    ...
    describe("reaction detection", () => {
        ...
        it("detects emoji://reaction as reaction", () => {
            ...
            const result = detectPattern(link, DEFAULT_CHAT_PREDICATES);
            assert.equal(result.type, "reaction");
            assert.equal(result.contentUri, "❤️");
        });
    });
    ...
    describe("content detection", () => {
        it("detects sioc://content_of as content when NOT in chatPredicates", () => {
            ...
            // Remove sioc://content_of from chat predicates
    ...
    // Priority: chat predicates take precedence over content detection
    describe("priority ordering", () => {
        it("chat predicate takes priority over content_of detection", () => {
            const link = makeLink({
                predicate: "sioc://content_of",
            });
            // sioc://content_of is in both chatPredicates and CONTENT_PREDICATE
            // chatPredicates check comes first
            const result = detectPattern(link, DEFAULT_CHAT_PREDICATES);
            assert.equal(result.type, "chat-message");
        });

        it("reply predicate takes priority when predicate is also a chat predicate", () => {
            const link = makeLink({
                predicate: "flux://has_reply",
            });
            // Even if flux://has_reply were in chat predicates (unlikely),
            // reply check comes after chat but flux://has_reply is not
            // in DEFAULT_CHAT_PREDICATES, so it hits reply first
            const result = detectPattern(link, DEFAULT_CHAT_PREDICATES);
            assert.equal(result.type, "reply");
        });
    });
    ...
    describe("edge cases", () => {
        ...
        it("handles empty chatPredicates array", () => {
            ...
            // flux://has_message is not in reply/mention/reaction/content
            assert.equal(result.type, "unknown");
        });
